...
import { getWhere, getStar, getKeyword } from '../redux/modules/searchValue'
...
function MainSearch() {
    ...
    const clickRadioHandler = (e) => {
        starRadio(e.target.value)
    }


    // const [a, setA] = useState();
    // <option value={a} onChange={(e) => setA(e.target.value)}/>
    // dispatch(getwhere(a))

    const [keyword, setKeyword] = useState('')

    const dispatch = useDispatch()
    const reSelectWhere = useSelector((state) => state.search.selectWhere)
    console.log('selectWhere:', reSelectWhere)
    ...
    return (
        ...
        //         </sst.Row></div>
            <div><span className='spanBold'>위치 선택</span>
                <sst.SelectStyle options={searchWhere} onChange={(e) => setSelectWhere(e.target.value)} defaultValue={searchWhere[0]} />
            </div>
        //     <sst.Row>
